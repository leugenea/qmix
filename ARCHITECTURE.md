...
|---|---|---|
| Backend | Go | Один бинарник, простота деплоя, хорошая поддержка HTTP/SSE; резолвинг ссылок — synchro |
...
базе `synchro`. Дополнительные зависимости — только проверенные и
закрывающие задачу целиком.
...
```
Реализации: VK, Яндекс Музыка, Spotify — на базе библиотеки
`oklookat/synchro`. Выбор по домену ссылки. Ошибки нерезолвимых ссылок
