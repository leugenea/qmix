...
`expires_in=0` означает, что VK выдал непротухающий токен (scope `offline`
реально применён). Ненулевое значение — токен временный: в password grant
VK игнорирует запрошенный `scope` и возвращает собственную маску прав
приложения, поэтому `offline` не гарантирован. В этом случае утилита пишет
предупреждение в stderr, чтобы токен не отвалился позже с невнятной
ошибкой авторизации.
