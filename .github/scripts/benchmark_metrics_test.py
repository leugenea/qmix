...
class BenchmarkMetricsTest(unittest.TestCase):
    def report(self):
        return {"schema_version": 1,
                "overall": {"erosion": 0.125, "high_ccn_count": 3},
                "languages": {"Go": {"erosion": 0.75},
                              "Kotlin": {"erosion": 0.0025},
                              "JS": {"erosion": 0, "mass": 0, "function_count": 0}}}

    def test_five_named_smaller_is_better_metrics_and_units(self):
        self.assertEqual(benchmark_metrics.metrics_from_lizard(self.report()), [
            {"name": "Overall erosion", "unit": "%", "value": 12.5},
            {"name": "Go erosion", "unit": "%", "value": 75.0},
            {"name": "Kotlin erosion", "unit": "%", "value": 0.25},
            {"name": "JS erosion", "unit": "%", "value": 0.0},
            {"name": "Functions with CCN > 10", "unit": "functions", "value": 3},
        ])

    def test_small_positive_erosion_is_not_rounded_to_zero(self):
        report = self.report()
        report["languages"]["JS"]["erosion"] = 1e-10
        self.assertGreater(benchmark_metrics.metrics_from_lizard(report)[3]["value"], 0)
    ...
    def test_cli_writes_one_generic_json_array(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(json.loads(output.read_text()),
                             benchmark_metrics.metrics_from_lizard(self.report()))

    def test_rejects_malformed_report_and_values(self):
        for change in (
            lambda r: r.update(schema_version=2),
            lambda r: r["languages"].pop("Go"),
            lambda r: r["overall"].update(erosion=float("nan")),
            lambda r: r["overall"].update(erosion=-0.01),
            lambda r: r["overall"].update(erosion=1.01),
            lambda r: r["languages"]["JS"].update(erosion=True),
            lambda r: r["overall"].update(high_ccn_count=-1),
            lambda r: r["overall"].update(high_ccn_count=1.2),
        ):
            report = self.report()
            change(report)
            with self.subTest(report=report), self.assertRaises((ValueError, KeyError)):
                benchmark_metrics.metrics_from_lizard(report)
