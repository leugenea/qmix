...
class PublicReadinessPolicyTest(unittest.TestCase):
    ...
    def test_clone_gate_uses_jscpd_baseline_on_pr_base(self):
        ci = read(".github/workflows/ci.yml")
        gate = job(ci, "clone-gate")
        result = job(ci, "result")
        self.assertIn("name: clone-gate", gate)
        self.assertIn("needs: route", gate)
        self.assertIn("github.event_name == 'pull_request'", gate)
        self.assertIn("needs.route.outputs.duplication == 'true'", gate)
        ...
        self.assertIn("v5.3.2/jscpd-linux-x64-gnu.tar.gz", gate)
        self.assertIn("'97259f222ea7f6d51a0f2faa98ed5889430233e0fb8d2c129f23d84aff4b93b2' 'jscpd-linux-x64-gnu.tar.gz' | sha256sum -c -", gate)
        self.assertLess(gate.index("sha256sum -c -"), gate.index("tar -xzf"))
        self.assertEqual(gate.count('"$RUNNER_TEMP/jscpd/jscpd"'), 1)
        self.assertEqual(gate.count("--reporters json --threshold 100 --fail-on-empty"), 1)
        self.assertIn("jscpd_report.py sources .", gate)
        self.assertIn('--baseline-from-ref "$BASE_SHA" --fail-on-new-clones', gate)
        self.assertIn('printf \'%s\\n\' "$status" > "$RUNNER_TEMP/clone-gate/scan-status"', gate)
        self.assertIn("--scan-status", gate)
        self.assertNotIn("git worktree add", gate)
        self.assertIn("id: scan", gate)
        self.assertIn("if: always()", gate)
        self.assertIn('cat "$RUNNER_TEMP/clone-gate/report.md" >> "$GITHUB_STEP_SUMMARY"', gate)
        self.assertIn("name: clone-gate", gate)
        self.assertIn("clone-gate/report.json", gate)
        self.assertIn("if-no-files-found: error", gate)
        self.assertNotIn("clone-gate", result)
